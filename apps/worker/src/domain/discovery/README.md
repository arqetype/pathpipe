...
before the crawl: `seed.ts` finds boards across every vendor and registers the
companies behind them.

It runs on its own schedule — `WORKERS_SCRAPE_SEED_CRON`, daily at 03:00 by
default — from the `--feeds` and `--companies` sources, so the board list
refills itself. `scripts/seed-boards.ts` is the same code behind a command
line, for a run by hand; there is one implementation, not two.
...
`--feeds` reads four public job feeds — Himalayas, Arbeitnow, Remote OK and
Jobicy — for the names of companies hiring right now. None of them exposes the
employer's own board URL (every one links back to itself), so what they are
worth here is the roster: the probe against the vendor API is still what decides
whether a name is a board. A 400-name sample confirmed 139 boards.

A board answers to its name whatever the case, so `everai` and the company's own
`EverAI` both confirm — the seeder keeps one hit per board, or each spelling
would be registered as a separate company.
...
is grown rather than regenerated by hand. Re-running is always safe: seeding is
an upsert server-side.
...
`detectInHtml(html)`, and `fetch(target, ctx)` returning `DiscoveredJob[]` — then
