...
export const stripHtml = (value: string): string =>
  decodeEntities(
    ...
    .replace(/[ \t ]+/g, ' ')
    // A block's opening tag became a space and its closing one a newline, so
    // without this every line but the first starts with a stray space — and a
    // blank line carrying one never matches the collapse below.
    .replace(/ ?\n ?/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
